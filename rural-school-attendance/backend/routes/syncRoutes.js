...
router.post('/', async (req, res, next) => {
  try {
    ...
    const pmPoshanUrl = process.env.PM_POSHAN_API_URL || 'https://mock-api.pmposhan.gov.in/students';
    const ssaUrl = process.env.SSA_API_URL || 'https://mock-api.ssa.gov.in/attendance';

    const pmPoshanResponse = await axios.get(pmPoshanUrl);
    const ssaResponse = await axios.get(ssaUrl);
